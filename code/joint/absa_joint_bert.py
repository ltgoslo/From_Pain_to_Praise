...
def classify_aspect(threshold:float, lr:float, batch:int, size:str, train_domain:str, test_domain:str, 
    ...
    os.environ["WANDB_PROJECT"] = "absa_joint" 
    ...
    aspects = []
    true_direc = f"../data/joint_ds/{test_domain}"
    
    with open(f"{true_direc}/full_ds.json", "r", encoding="utf-8") as readfile:
    ...
    def compute_metrics(p):
        ...
        # writing predictions
        if predicting:
            true_path = f"../data/{true_direc}/test.json"
            key = "test"
        else:
            true_path = f"../data/{true_direc}/dev.json"
            key = "dev"
    ...
    # datasets
    train_dir = f"../data/hf_arrow_joint/{train_domain}"
    test_dir = f"../data/hf_arrow_joint/{test_domain}"
    ...
    training_args = TrainingArguments(
        output_dir=f"./nb3_joint_results/{train_domain}_{test_domain}_{size}/full_{seed}",
        eval_strategy="epoch",
        save_strategy="no",
    ...
    # create results dir
    os.makedirs(f"./nb3_joint_results/{train_domain}_{test_domain}_{size}/full_{seed}", exist_ok=True)
    training_args.output_dir = f"./nb3_joint_results/{train_domain}_{test_domain}_{size}/full_{seed}"
    ...
    os.makedirs("model_path/joint_model")
    tokenizer.save_pretrained(f"model_path/joint_model/{train_domain}_{seed}")
    trainer.save_model(output_dir=f"model_path/joint_model{train_domain}_{seed}")
